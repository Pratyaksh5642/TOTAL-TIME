...
# File handler gets EVERYTHING (DEBUG level - includes ignored items)
file_handler = logging.FileHandler(LOG_FILE, mode='w', encoding='utf-8')
file_handler.setLevel(logging.DEBUG)
file_handler.setFormatter(formatter)
logger.addHandler(file_handler)

# Console handler gets ONLY INFO level (hides ignored items from terminal)
console_handler = logging.StreamHandler()
console_handler.setLevel(logging.INFO)
console_handler.setFormatter(formatter)
logger.addHandler(console_handler)

# Mute the noisy "GET HTTP/1.1" logs from the requests library!
...
def process_hierarchy(work_item_url, visited=None, depth=0, is_rework_branch=False):
    ...
    if time_spent_ms > 0:
        ...
        if bucket:
            ...
            # Logs to file ONLY
            logger.debug(f"{indent}→ [IGNORED CATEGORY] Skipped {hours_logged:.2f} hrs | ID: {item_id} | Dept: '{task_department_name}'")
...
if __name__ == "__main__":
    logger.info("\n---> [RUNNING LOGGED & FILTERED VERSION] <---")
            ...
            for row in reader:
                ...
                if release_data:
                    ...
                    if "invalid" in res_status:
                        ...
                        logger.info(f"✔️ Resolution is '{res_status.title()}'. Processing Hierarchy...")
