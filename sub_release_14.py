...
LOG_FILE = os.path.join(SCRIPT_DIR, "extraction_log.txt")         # Master log (Everything)
...
def process_hierarchy(work_item_url, visited=None, depth=0, is_rework_branch=False):
    ...
    if time_spent_ms > 0:
        ...
        if bucket:
            ...
            # INFO LEVEL -> Goes to console AND BOTH log files
            logger.info(f"{indent}→ [{bucket} {rework_str}] Added {hours_logged:.2f} hrs | ID: {item_id} | Type: {type_name_short} | Dept: '{task_department_name}' | Country: {task_country}")
        else:
            # DEBUG LEVEL -> Goes ONLY to the master extraction_log.txt
...
if __name__ == "__main__":
    logger.info("\n---> [NEW RUN STARTING: DUAL LOG FILES ENABLED] <---")
            ...
            for row in ordered_rows:
                ...
                if response and response.status_code == 200:
                    ...
                    if not resolved_raw or not isinstance(resolved_raw, str):
                        ...
                        resolved_raw = match.group(1) if match else None
