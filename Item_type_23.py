...
INPUT_CSV_FILE = os.path.join(SCRIPT_DIR, "2023_2024_release_DAG_new.csv")
OUTPUT_EXCEL_FILE = os.path.join(SCRIPT_DIR, "2023_2024.xlsx") 
LOG_FILE = os.path.join(SCRIPT_DIR, "extraction_log_2023_2024.txt")
MAPPING_CSV_FILE = os.path.join(SCRIPT_DIR, "mapping.csv")
ADDED_LOG_FILE = os.path.join(SCRIPT_DIR, "added_time_log_2023_2024.txt")
...
def load_team_roster():
    """Scans the Excel file and extracts ONLY the columns strictly labeled as Names."""
        ...
        try:
            ...
            for sheet in xls.sheet_names:
                ...
                for col in df.columns:
                    ...
                    if 'name' in col_str or 'resource' in col_str:
                        name_col = col
                        break
                
                # STRICT FIX: We removed the fallback to df.columns[0]. 
                # If there is no "Name" column on this sheet, it skips the sheet entirely!
                
                if name_col is not None:
                    for val in df[name_col].dropna().astype(str):
                        clean_val = val.strip().lower()
                        if clean_val and len(clean_val) > 2 and clean_val != str(name_col).strip().lower():
                            TEAM_ROSTER.add(clean_val)
                            
            logger.info(f"✔️ Loaded {len(TEAM_ROSTER)} unique names from '{TEAM_ROSTER_FILE}' for Rescue Operations.")
...
if __name__ == "__main__":
    logger.info("\n---> [NEW RUN STARTING: STRICT ROSTER EXTRACTION] <---")
