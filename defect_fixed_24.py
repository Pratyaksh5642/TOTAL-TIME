...
INPUT_CSV_FILE = os.path.join(SCRIPT_DIR, "Release ID_DAG.csv")
OUTPUT_EXCEL_FILE = os.path.join(SCRIPT_DIR, "Final_Excel.xlsx") 
LOG_FILE = os.path.join(SCRIPT_DIR, "extraction_log_Final.txt")
MAPPING_CSV_FILE = os.path.join(SCRIPT_DIR, "mapping.csv")
ADDED_LOG_FILE = os.path.join(SCRIPT_DIR, "added_time_log_Final.txt")
...
def load_team_roster():
    """Scans the Excel file and extracts ONLY the 'Names' column."""
    if os.path.exists(TEAM_ROSTER_FILE):
        try:
            ...
            if not sheets_used:
                ...
                logger.info(f"✔️ Loaded {len(TEAM_ROSTER)} unique names from sheets {sheets_used} in '{TEAM_ROSTER_FILE}' for Rescue Operations.")
...
def process_single_release(row):
    ...
    if response and response.status_code == 200:
        ...
        # --- FIXED: VIP PASS FOR ROOT DEFECTS ---
...
if __name__ == "__main__":
    logger.info("\n---> [NEW RUN STARTING: ROOT ITEM VIP PASS FIXED] <---")
