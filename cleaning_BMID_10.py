#added Fallback logic for mapping
...
def get_bucket_for_category(category_name):
    """STRICT MAPPING: Uses CSV mapping first. If mapped to NA, ignores. If missing entirely, falls back."""
    name = category_name.lower()
    
    # 1. Check if it explicitly exists in the CSV
    if name in CATEGORY_MAPPING:
        bucket = CATEGORY_MAPPING[name]
        if bucket in ["NET", "DCOM", "DSM"]:
            return bucket
        elif bucket == "NA":
            return None # Explicitly ignore, DO NOT fall back
        else:
            return None # Ignore any weird typos in the CSV
            
    # 2. Fallback to Keyword Search (ONLY if missing from CSV completely)
    if any(kw in name for kw in ["net", "fota", "var", "dpe", "hmi"]):
        ...
        return "DSM" # Maps "dem" and others to DSM bucket
        
    # 3. If no match anywhere, ignore
    return None 
...
if __name__ == "__main__":
    logger.info("\n---> [NEW RUN STARTING: STRICT MAPPING WITH NA IGNORE] <---")
