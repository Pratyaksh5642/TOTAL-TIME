...
def process_hierarchy(work_item_url, release_id, blog, visited=None, depth=0, is_rework_branch=False):
    ...
    if time_spent_ms > 0:
        ...
        if not is_valid_type:
            ...
            blog.debug(f"[Rel {release_id}] {indent}→ [IGNORED CATEGORY] Skipped {hours_logged:.2f} hrs | ID: {item_id} | Type: {type_name_short} | Dept: '{task_department_name}' | Title: {task_title} | Created: {task_created_formatted} | Resolved: {task_resolved_formatted}")
