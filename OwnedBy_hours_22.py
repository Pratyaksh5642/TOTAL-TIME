...
OUTPUT_EXCEL_FILE = os.path.join(SCRIPT_DIR, "check_new.xlsx") 
    ...
    if os.path.exists(TEAM_ROSTER_FILE):
        try:
            ...
            for sheet in xls.sheet_names:
                ...
                # Check directly for the "Names" column from the provided file
                name_col = None
                ...
                # If no 'Names' header found, safely assume the first column
...
if __name__ == "__main__":
    logger.info("\n---> [NEW RUN STARTING: TEAM ROSTER EXACT COLUMN MATCHING] <---")
    logger.info(f"Master Log: {LOG_FILE}")
    logger.info(f"Clean Log (Added Only): {ADDED_LOG_FILE}\n")
    
    load_category_mapping()
    load_team_roster() # <--- Loads names into memory before threads start
