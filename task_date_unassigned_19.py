...
def process_hierarchy(work_item_url, visited=None, depth=0, is_rework_branch=False):
    ...
    if time_spent_ms > 0:
        ...
        # --- NEW: Extracting Task Resolution Date ---
        ...
        if not is_valid_type:
            logger.debug(f"{indent}→ [IGNORED TYPE] Skipped {hours_logged:.2f} hrs | ID: {item_id} | Type: {type_name_short} | Title: {task_title}")
            ...
            # --- NEW: Appended Resolution Date to logs ---
            logger.info(f"{indent}→ [{bucket} {rework_str}] Added {hours_logged:.2f} hrs | ID: {item_id} | Type: {type_name_short} | Dept: '{task_department_name}' | Owner: {task_owner_string} | Country: {task_country} | Title: {task_title} | Created: {task_created_formatted} | Resolved: {task_resolved_formatted}")
        else:
            logger.debug(f"{indent}→ [IGNORED CATEGORY] Skipped {hours_logged:.2f} hrs | ID: {item_id} | Type: {type_name_short} | Dept: '{task_department_name}' | Title: {task_title} | Created: {task_created_formatted} | Resolved: {task_resolved_formatted}")
...
if __name__ == "__main__":
    logger.info("\n---> [NEW RUN STARTING: TASK RESOLUTION & UNASSIGNED FIX] <---")
                ...
                if response and response.status_code == 200:
                    ...
                    if "invalid" in res_status.lower():
                        ...
                        if res_url:
                            ...
                            logger.info(f"✔️ Status is '{res_status}' (No resolution yet).")
