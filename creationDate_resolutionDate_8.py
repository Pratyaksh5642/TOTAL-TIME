# Added creation date resolution date, Changed DEM TO DSM and added a total of DCOM_DSM_TOTAL for Rework and total and NET_FINAL, DCOM_DSM_FINAL
...
def format_date(iso_date_str):
    """Converts YYYY-MM-DDTHH:MM:SSZ to DD-MM-YYYY"""
    if not iso_date_str:
        return "Not Found"
    try:
...
def extract_date_field(json_data, field_keyword):
    """Deep search JSON for a date field like 'resolved' or 'created'"""
...
def process_hierarchy(work_item_url, visited=None, depth=0, is_rework_branch=False):
    ...
    efforts = {
        "NET_Total": 0, "DCOM_Total": 0, "DSM_Total": 0,
...
if __name__ == "__main__":
    logger.info("\n---> [RUNNING VERSION: RESTORED MODIFIED FALLBACK FOR DATE] <---")
        ...
        with open(INPUT_CSV_FILE, mode="r", encoding="utf-16") as infile:
            ...
            for row in reader:
                ...
                if release_data:
                    ...
                    # 2. Extract Owner Name String & Calculate Country/Rate
                    ...
                    # 3. Extract and Format Dates (WITH MODIFIED FALLBACK RESTORED)
                    created_raw = extract_date_field(release_data, "created")
                    
                    # Try to find "resolved" first
                    resolved_raw = extract_date_field(release_data, "resolved")
                    # If ALM API hid it, fallback to the last "modified" date!
                    if not resolved_raw:
                ...
                # --- CALCULATE ALL HOURS ---
                net_tot = efforts_ms["NET_Total"] / 3600000
                dcom_tot = efforts_ms["DCOM_Total"] / 3600000
                dsm_tot = efforts_ms["DSM_Total"] / 3600000
                
                net_rew = efforts_ms["NET_Rework"] / 3600000
                dcom_rew = efforts_ms["DCOM_Rework"] / 3600000
                dsm_rew = efforts_ms["DSM_Rework"] / 3600000
                
                # --- LOG ONLY THE BASE 6 BUCKETS TO TERMINAL ---
                if net_tot > 0: logger.info(f"  NET_Total: {net_tot:.2f} hrs")
                if dcom_tot > 0: logger.info(f"  DCOM_Total: {dcom_tot:.2f} hrs")
                if dsm_tot > 0: logger.info(f"  DSM_Total: {dsm_tot:.2f} hrs")
                ...
                # --- CALCULATE FINALS ---
                net_final = net_tot + net_rew
                dcom_dsm_tot = dcom_tot + dsm_tot
                dcom_dsm_rew = dcom_rew + dsm_rew
                dcom_dsm_final = dcom_dsm_tot + dcom_dsm_rew

                # --- ASSIGN IN STRICT ORDER TO CSV ---
                row["NET_Total (Hours)"] = round(net_tot, 2)
                row["DCOM_Total (Hours)"] = round(dcom_tot, 2)
                row["DSM_Total (Hours)"] = round(dsm_tot, 2)
                
                row["NET_FINAL (Hours)"] = round(net_final, 2)
                row["DCOM_DSM_FINAL (Hours)"] = round(dcom_dsm_final, 2)
                row["DCOM_DSM_TOTAL (Hours)"] = round(dcom_dsm_tot, 2)
