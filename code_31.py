...
# --- OEM MAPPING ---
OEM_MAPPING = {
    "cus toy-dai - production": "Toyota LV",
    "cus dag esp9.3 - production": "Mercedes-B",
    "cus mb": "Mercedes-B",
    "cus gm - production": "GM",
    "cus gm opel - production": "GM",
...
ROSTER_MAPPING = {
    "cus audi - production": "EHE",
    "cus joem esp9.3 - production": "EHA",
    "cus honda - production": "EHA",
    "cus mazda - production": "EHA",
    "cus nissan gen9.3 - production": "EHA",
    "cus gm opel - production": "EHN",
    "cus coem esp9.3 - production": "EHN",
    "cus toy-dai - production": "EHA",
    "cus porsche - production": "EHE",
    "cus jlr - production": "EHE",
    "cus volvo - production": "EHE",
    "cus vag - production": "EHE",
    "cus gm - production": "EHN",
    "cus fiat - production": "EHN",
    "cus fr oem - production": "EHN",
    "cus ford gen9.3 - production": "EHN",
    "cus dag esp9.3 - production": "EHE",
    "cus mb": "EHE",
    "cus gm sa_sdv - production": "EHN"
}
...
KNOWN_USERS = {}
CATEGORY_MAPPING = {}
TEAM_ROSTERS = {} # NEW: Changed to a dictionary to hold separate teams
    ...
    if os.path.exists(TEAM_ROSTER_FILE):
        try:
            ...
            for sheet in xls.sheet_names:
                ...
                TEAM_ROSTERS[sheet_key] = set() # Create a blank set for this sheet/team
                    ...
                    if clean_val and len(clean_val) > 2 and clean_val != 'names':
                        ...
                        if len(words) <= 6:
                            for perm in itertools.permutations(words):
                                TEAM_ROSTERS[sheet_key].add(" ".join(perm))
                        else:
                            TEAM_ROSTERS[sheet_key].add(clean_val)
                            
            if not sheets_used:
                logger.warning(f"⚠️ No sheet in '{TEAM_ROSTER_FILE}' has a 'Names' column! Rescue feature will be skipped.")
            else:
                total_combos = sum(len(s) for s in TEAM_ROSTERS.values())
                logger.info(f"✔️ Loaded {total_combos} name combinations across {len(sheets_used)} teams {sheets_used} in '{TEAM_ROSTER_FILE}'.")
...
def is_owner_in_roster(owner_string, target_sheet):
    """Checks if the extracted owner name exists in the SPECIFIC team roster."""
    if not target_sheet or target_sheet not in TEAM_ROSTERS:
        return False # We don't know what team to check, so we can't rescue them.
        
    roster_set = TEAM_ROSTERS[target_sheet]
    
    if not roster_set or not owner_string or owner_string == "Unassigned":
        return False
        
    owner_lower = owner_string.lower()
    name_part = owner_lower.split('(')[0].strip()
    if name_part in roster_set:
        return True
        
    for r_name in roster_set:
...
def process_hierarchy(work_item_url, release_id, pm_id, oem, planned_for, extra_cols_data, root_type_short, res_status, release_owner_string, 
                      created_formatted, resolved_formatted, target_sheet, blog, visited=None, depth=0, is_rework_branch=False):
    if visited is None:
        visited = set()
    ...
    if time_spent_ms > 0:
        ...
        if skip_due_to_invalid:
            ...
            # Use the target_sheet parameter to check ONLY the specific team roster!
            if not bucket and is_owner_in_roster(task_owner_string, target_sheet):
                bucket = "GENERAL"
                task_department_name = f"{task_department_name} (Rescued via Roster {target_sheet})"
            ...
            if bucket:
                ...
                task_dict.update(extra_cols_data)
    ...
    for child in children_data:
        if isinstance(child, dict) and "rdf:resource" in child:
            ...
            child_efforts, child_tasks = process_hierarchy(
                child_url, release_id, pm_id, oem, planned_for, extra_cols_data, root_type_short, res_status, release_owner_string, 
                created_formatted, resolved_formatted, target_sheet, blog, visited, depth + 1, current_is_rework
            )
...
def process_single_release(row, extra_columns):
    ...
    project_area = row.get("Project Area", "").strip()
    # Map to OEM name
    oem = OEM_MAPPING.get(project_area.lower(), project_area)
    # Map to specific Roster Sheet (EHE, EHN, EHA)
    target_sheet = ROSTER_MAPPING.get(project_area.lower(), None)
    
    planned_for = row.get("Planned For", "").strip()
    
    extra_cols_data = {col: str(row.get(col, "")).strip() for col in extra_columns}
    ...
    efforts_by_country, task_details = process_hierarchy(
        root_url, release_id, pm_id, oem, planned_for, extra_cols_data, root_type_short, res_status, release_owner_string, 
        created_formatted, resolved_formatted, target_sheet, blog
    )
...
if __name__ == "__main__":
    logger.info("\n---> [NEW RUN STARTING: SHEET-SPECIFIC ROSTER MAPPING & SUMMARY ROWS] <---")
    ...
    try:
        with open(INPUT_CSV_FILE, mode="r", encoding="utf-16") as infile:
            ...
            known_input_cols = {"id", "pm interface element id", "project area", "planned for"}
            extra_columns = [col for col in fieldnames if col and col.strip().lower() not in known_input_cols]
            
            all_rows = list(reader)
            
            unique_ids = set()
            unique_planned_for = set()
            ordered_rows = []
            ...
            for row in all_rows:
                ...
                unique_ids.add(release_id)
                pf_val = row.get("Planned For", "").strip()
                if pf_val:
                    unique_planned_for.add(pf_val)
            ...
            logger.info(f"Reading complete. Found {len(unique_ids)} Unique IDs and {len(unique_planned_for)} Unique Planned For entries.")
            logger.info(f"Starting ThreadPoolExecutor with 30 threads...\n")
            
            with ThreadPoolExecutor(max_workers=30) as executor:
                future_to_row = {executor.submit(process_single_release, row, extra_columns): row for row in ordered_rows}
        ...
        if all_task_details:
            detail_columns = [
                "Release ID", "PM ID", "OEM", "Planned For", "Root Item Type", "Release Status", "Release Owner", 
            ...
            # Append the summary rows safely to the very bottom
            all_task_details.append({"Release ID": "BaseLine", "PM ID": len(unique_planned_for)})
            all_task_details.append({"Release ID": "Release ID Count", "PM ID": len(unique_ids)})
            all_task_details.append({"Release ID": "No Hours", "PM ID": ZERO_HOURS_COUNT})

            detail_df = pd.DataFrame(all_task_details, columns=detail_columns).fillna("")
