...
INPUT_CSV_FILE = os.path.join(SCRIPT_DIR, "Fordcheck.csv")
OUTPUT_EXCEL_FILE = os.path.join(SCRIPT_DIR, "testing.xlsx")
LOG_FILE = os.path.join(SCRIPT_DIR, "Extraction_NEW_Log_Honda_Final_with_countOfTask.txt")
ADDED_LOG_FILE = os.path.join(SCRIPT_DIR, "test.txt")
    ...
    if time_spent_ms > 0:
        ...
        if skip_due_to_invalid:
            ...
            if bucket:
                ...
                # Inject any dynamically discovered extra columns
...
def process_single_release(row, extra_columns):
    ...
    # Extract dynamic inputs for OEM mapping and Planned For
    ...
    # Grab all extra unexpected columns dynamically
...
if __name__ == "__main__":
    logger.info("\n---> [NEW RUN STARTING: OEM MAPPING + DYNAMIC COLUMNS + SUMMARY ROWS] <---")
    ...
    try:
        with open(INPUT_CSV_FILE, mode="r", encoding="utf-16") as infile:
            ...
            # --- FEATURE 1: DYNAMIC COLUMNS DETECTION ---
            ...
            # Global Trackers for Summary Rows
            unique_ids = set()
            ...
            with ThreadPoolExecutor(max_workers=30) as executor:
                # Pass the dynamically detected extra columns to the worker
        ...
        if all_task_details:
            # Build the exact column list, placing Extra Columns at the far right
            detail_columns = [
            ...
            # --- FEATURE 2: APPEND SUMMARY ROWS DIRECTLY TO THE EXCEL SHEET ---
            ...
            # Create dataframe and replace any completely missing values with empty strings
