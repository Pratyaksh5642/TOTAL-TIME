#test in filed against will also be dropped
...
def get_bucket_for_category(category_name):
    ...
    # --- NEW: 'test' Keyword Filter ---
    # If not in CSV, explicitly ignore if it contains 'test'
    if "test" in name:
        return None

    # 2. Proceed to standard keyword fallback
...
def process_hierarchy(work_item_url, visited=None, depth=0, is_rework_branch=False):
    ...
    if time_spent_ms > 0:
        ...
        # Check constraints before adding time
...
if __name__ == "__main__":
    logger.info("\n---> [NEW RUN STARTING: TYPE & TEST FILTERS ADDED] <---")
